...
LOG="/workspace/.build-monitor-v5.log"

echo "[$(date -u)] Monitoring #$RUN_ID" | tee "$LOG"
...
while true; do
    ...
    echo "[$NOW] status=$STATUS conclusion=$CONCLUSION" | tee -a "$LOG"
    
    if [ "$STATUS" = "completed" ]; then
        if [ "$CONCLUSION" = "success" ]; then
            echo "[$NOW] SUCCESS" | tee -a "$LOG"
            exit 0
        else
            echo "[$NOW] FAILED: $CONCLUSION" | tee -a "$LOG"
            exit 1
        fi
    fi
    sleep 600
